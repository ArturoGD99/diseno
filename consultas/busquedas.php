...
if($tp == 'cte'){
    ...
    if($rs->RecordCount()>0){
        ...
        echo $id."@".$ind."@".$ind2."@".$ind3."@".$ind4."@".$ind5;
    }else echo 0;
}else if($tp == 'corrida'){
    $tbl = "";
    $ficha = $_POST['ficha'];
    ...
    $mc = $_POST['mc'];
    $rs=$query->Consultar($campos,$tablas,$criterios,"");
    echo "<select class='custom-select' id='cmborden' name='ncmborden' tabindex='4'><option value='0'></option>";//onchange='Buscar_Prendas()'
    echo "<optgroup>";
    while(!$rs->EOF){
        echo "<option value='".$rs->fields['ID_FICHA']."' ".($mc==$rs->fields['ID_FICHA']?"selected":"").">".$rs->fields['ORDEN']."</option>";
        $rs->MoveNext();
    }
    echo "</optgroup>";
    echo "</select>";
